...
To achieve this, `rook-ceph` will be used as default `StorageClass` and we will pre-provision a PVC for `mariadb-repl-1` by matching the `PVC` name expected by the `StatefulSet`: `storage-<mariadb-name>-<pod-index>`.
